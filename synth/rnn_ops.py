...
def _concat(prefix, suffix):
    """Concat prefix (scalar/1D) with suffix (TensorShape/tensor) into int32 1D tensor."""
    if isinstance(prefix, tf.Tensor):
        p = array_ops.reshape(prefix, [-1])
    else:
        p = [prefix] if isinstance(prefix, int) else list(prefix)
        p = constant_op.constant(p, dtype=dtypes.int32)
    ...
    if isinstance(suffix, (list, tuple)):
        ...
        s = array_ops.reshape(tf.cast(suffix, dtypes.int32), [-1])
    else:
        s = constant_op.constant([suffix], dtype=dtypes.int32)

    return array_ops.concat([tf.cast(p, dtypes.int32), tf.cast(s, dtypes.int32)], 0)


def _like_rnncell(cell):
    """Check if cell looks like an RNNCell."""
    return hasattr(cell, 'state_size') and hasattr(cell, 'output_size')


def _maybe_tensor_shape_from_tensor(t):
