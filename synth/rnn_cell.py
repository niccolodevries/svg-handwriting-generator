...
class LSTMAttentionCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        with tf.variable_scope(scope or type(self).__name__, reuse=tf.AUTO_REUSE):
            ...
            s1_out, s1_state = cell1(s1_in, state=tf.nn.rnn_cell.LSTMStateTuple(state.c1, state.h1))
            ...
            s2_out, s2_state = cell2(s2_in, state=tf.nn.rnn_cell.LSTMStateTuple(state.c2, state.h2))

            # lstm 3
            s3_in = tf.concat([inputs, s2_out, w], axis=1)
            cell3 = tf.nn.rnn_cell.LSTMCell(self.lstm_size)
            s3_out, s3_state = cell3(s3_in, state=tf.nn.rnn_cell.LSTMStateTuple(state.c3, state.h3))
